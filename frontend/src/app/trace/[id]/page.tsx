...
function TraceContent() {
    ...
    const fetchDNA = async () => {
        try {
            setLoading(true);
            // In a real app, this would be a dynamic DNA fetch
            // For now, we mock it or fetch from the specific ID
            const res = await fetch(getApiUrl(`/api/horizon/dna/${productId || id}`));
            if (res.ok) {
                setPassport(await res.json());
            } else {
                // Fallback mock for demonstration if API fails or is empty
                setPassport({
                    id: id,
