...
// --- 🛰️ INTEL MANAGEMENT (VAULT & EXCHANGE) ---

// 1. GET /api/admin/intel - Fetch all intelligence nodes
router.get('/intel', async (req, res) => {
    try {
        const { section } = req.query;
        let query = db.select().from(intelContents);

        if (section) {
            query = query.where(eq(intelContents.section, section));
        }

        const data = await query.orderBy(desc(intelContents.createdAt));
        res.json(data);
    } catch (error) {
        console.error('Fetch Intel Error:', error);
        res.status(500).json({ error: 'Failed to synchronize intelligence registry' });
    }
});

// 2. POST /api/admin/intel - Initialize a new intelligence node
router.post('/intel', async (req, res) => {
    try {
        const { title, body, type, category, section, status } = req.body;
        const [newNode] = await db.insert(intelContents).values({
            title,
            body,
            type,
            category,
        ...
        res.status(500).json({ error: 'Failed to propagate new intelligence node' });
    }
});

// 3. PATCH /api/admin/intel/:id - Update an intelligence node
router.patch('/intel/:id', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const [updatedNode] = await db.update(intelContents)
            .set({
                ...updateData,
                updatedAt: new Date()
            })
        ...
        if (!updatedNode) return res.status(404).json({ error: 'Intelligence node not found' });
        res.json(updatedNode);
    } catch (error) {
        console.error('Update Intel Error:', error);
        res.status(500).json({ error: 'Failed to reconfigure intelligence node' });
    }
});

// 4. DELETE /api/admin/intel/:id - CATASTROPHE: Wipe intelligence node
router.delete('/intel/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [deletedNode] = await db.delete(intelContents)
            .where(eq(intelContents.id, id))
            .returning();

        if (!deletedNode) return res.status(404).json({ error: 'Intelligence node not found' });
        res.json({ message: 'Intelligence node purged from global index' });
    } catch (error) {
        console.error('Delete Intel Error:', error);
        res.status(500).json({ error: 'Failed to erase intelligence node' });
    }
});
